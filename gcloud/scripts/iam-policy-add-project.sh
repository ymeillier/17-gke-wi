...
# ./scripts/iam-policy-add-project.sh "your-project-id" "user:account-email" "${my_roles[@]}"
#or for service account
# ./scripts/iam-policy-add-project.sh "your-project-id" "serviceAccount:s-aaccount-email" "${my_roles[@]}"

# IF user does not invoke the function with the right number of arguments:
if [ "$#" -lt 3 ]; then
    echo "⚠️"
    echo "Usage: $0 <PROJECT_ID> <ACCOUNT> <ROLE_1> [<ROLE_2> ...]"
    echo "Example: $0 \"my-gcp-project\" \"user:user@example.com\" \"roles/viewer\" \"roles/storage.objectViewer\""
    echo "Example: $0 \"my-gcp-project\" \"serviceAccount:sa@xyz.gserviceaccount.com\" \"roles/viewer\" \"roles/storage.objectViewer\""
    exit 1
fi
...
echo "Assigning roles to $MEMBER on project: $PROJECT_ID"
